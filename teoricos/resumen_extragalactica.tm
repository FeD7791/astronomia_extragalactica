...
<\body>
  ...
  A diferencia del brillo <math|I<rsub|\<nu\>>> el flujo depende de la
  distancia a la fuente: <math|S<rsub|\<nu\>>\<propto\>d<rsup|-2>>.

  Destaquese que el brillo <math|I<rsub|\<nu\>>> es una propiedad intrinseca
  de la fuente y por lo tanto no depende de la distancia.
  ...
  Observese que la luminosidad intrinseca es una propiedad de la fuente y no
  depende de la distancia ya que al ser <math|S<rsub|\<nu\>>> proporcional a
  <math|d<rsup|-2>> esta se cancela con <math|d<rsup|2>> en la expresion. Por
  otro lado esta expresion no es valida para fuentes no isotropicas como los
  quasars.
  ...
  <\itemize-dot>
    ...
    <item>El universo basicamente se compone de 5% materia barionica, 25%
    materia oscura y un 70% de energia oscura. Esta ultima esta vinculada a
    la constante cosmologica <math|\<Lambda\>> que introdujo Einstein.
    ...
    <item>En concreto nuestra galaxia parece contar con varias partes que
    pueden diferenciarse (y que pueden encontrarse en otras galaxias)

    <\itemize-minus>
      <item>Disco delgado de Estrellas con un radio promedio de 20kpc y una
      altura de 300pc

      <item>Un bulge Central

      <item>Un halo practicamente esferico, que contiene la mayoria de los
      clusteres globulares (<em|Un cúmulo globular es una conglomeración
      esferoidal de estrellas unidas por la gravedad , con una mayor
      concentración de estrellas hacia su centro. Puede contener desde
    ...
    diferentes propiedades vinculadas a su edad y su composicion quimica. Las
    estrellas mas jovenes suelen encontrarse en el disco mientras que la
    poblacion mas vieja se encuentra en el bulge. Se observa que el gas frio
    cae en el disco galactico, mientras que el gas caliente sale.
